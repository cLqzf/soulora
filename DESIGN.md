...
- Early-access research: a two-step progressive form first captures up to two intended-use moments and an optional short expectation, then asks for platform, email, and optional interview consent. The experience deliberately avoids names, demographics, and sensitive narratives. It must disclose when no secure collection endpoint is configured, and the production payload keeps research consent distinct from early-access contact.
